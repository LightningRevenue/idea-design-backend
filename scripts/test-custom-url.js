...
const BASE_URL =
  ...
  'https://idea-design-backend-fbfbcjg6grgvfght.westeurope-01.azurewebsites.net';
    ...
    try {
      ...
      if (response.status === 302) {
        ...
        if (
          ...
          response.headers.location.includes('rendertron.idea-design.ro')
        ) {
      ...
      if (error.response && error.response.status === 302) {
        console.log(
          `   ↗️  Redirecționat către: ${error.response.headers.location}`
        );

        if (
          error.response.headers.location &&
          error.response.headers.location.includes('rendertron.idea-design.ro')
        ) {
          console.log(`   ✅ Redirecționare către Rendertron!`);
        } else {
          console.log(`   ➡️  Redirecționare către frontend React`);
...
async function main() {
  ...
  console.log(
    `🌐 Backend URL: ${
      customPort ? `http://localhost:${customPort}` : BASE_URL
    }`
  );
